...
export class ChunkingService extends EventEmitter {
    ...
    protected async cleanupUploadState(metadata: UploadCleanupMetadata): Promise<void> {
        ...
        try {
            ...
            this.emit(UPLOAD_SOCKET_IO.EVENTS.CLEANUP, {
                trackingId,
                reason,
                error: error?.message,
                finalState: {
    ...
    protected async retryFailedChunks(
        ...
        for (const chunk of failedChunks) {
            ...
            this.emit(UPLOAD_SOCKET_IO.EVENTS.RETRY, {
                trackingId,
                chunkId: chunk.id,
    ...
    private emitProgress(trackingId: string, userId: string, file: formidable.File): void {
        ...
        this.emit(UPLOAD_SOCKET_IO.EVENTS.PROGRESS, event);
    }
}
...
export class UploadWebSocketHandler {
    ...
    private listenToChunkingService() {
        ...
        chunkingService.on(UPLOAD_SOCKET_IO.EVENTS.PROGRESS, (event: UploadProgress) => {
            this.notifyUser(event.userId, {
                type: UPLOAD_SOCKET_IO.EVENTS.PROGRESS,
                data: event
            });
        });

        chunkingService.on(UPLOAD_SOCKET_IO.EVENTS.CLEANUP, (event) => {
            this.notifyUser(event.trackingId, {
                type: UPLOAD_SOCKET_IO.EVENTS.CLEANUP,
                data: event
            });
        });
    }
