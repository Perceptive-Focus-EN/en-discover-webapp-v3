// src/components/EN/MoodHistoryChart.tsx
...
import { TimeRange, convertSourceIdsToNames } from './types/moodHistory';

interface MoodHistoryChartProps {
  emotion: Emotion;
  history: Emotion[];  // Changed from MoodHistoryItem[] to Emotion[]
  timeRange: TimeRange;
}
...
const MoodHistoryChart: React.FC<MoodHistoryChartProps> = ({ emotion, history, timeRange }) => {
  ...
  const aggregateData = (data: Emotion[]): AggregatedData[] => {
    const aggregated = data.reduce((acc, item) => {
      ...
      switch (timeRange) {
        ...
        case 'lifetime':
          ...
          break;
        default:
          key = item.createdAt || format(date, 'yyyy-MM-dd');
      }
    ...
    return Object.entries(aggregated).map(([key, value]) => ({
      date: key,
      volume: Math.round(value.totalVolume / value.count),
  ...
  const getVolumeName = (id: number): VolumeLevelName => {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <YAxis
            domain={[0, 4]}
            ticks={[0, 1, 2, 3, 4]}
            tickFormatter={(tick) => getVolumeName(tick)}
          />
